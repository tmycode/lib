...
class ConsoleUI extends react_1 {
    constructor(props) {
        super(props);
        this.state = {
            logs: [],
    ...
    componentDidMount() {
        let el = document.getElementById("consoleInput");
        let that = this;
        el.addEventListener("keypress", function (e) {
            if (e.keyCode == 13) {
                ...
                input.value = "";
                let log = { message: that.state.description + value };
                that.setState(({ logs }) => ({ logs: [log, ...logs] }));
                // iter.next(value);
                if (that.state.resolveCb)
                    that.state.resolveCb(value);
            }
        });
    }
    readPrompt(fieldDes) {
        return __awaiter(this, void 0, void 0, function* () {
            document.getElementById("consoleInput").placeholder = fieldDes;
            this.setState({ description: fieldDes });
            let that = this;
            return new Promise(function (resolve, reject) {
                that.setState(() => ({ resolveCb: resolve }));
            });
        });
    }
    result(message) {
        let log = { method: 'result', message };
        this.setState(({ logs }) => ({ logs: [log, ...logs] }));
    }
    clear() {
        this.setState(({ logs }) => ({ logs: [] }));
    }
    render() {
        return (react_5("div", { id: "demo", className: "fullsize overflow console" },
            react_5("div", { className: "topbar" },
                react_5("button", { className: "icon", onClick: this.clear.bind(this), title: "Clear" }, "\u2205"),
                react_5("input", { type: "text", id: "consoleInput", placeholder: "" })),
            react_5("ul", { style: { paddingTop: '5px' } }, this.state.logs.map((c, i) => react_5("li", { key: i }, c.message)))));
    }
}
...
class ConsoleApp {
    constructor() {
        this.instance = reactDom_1(react_5(ConsoleUI, null), document.getElementById('projectView'));
    }
    /**
     * read input with prompt
     * @param fieldDes
     */
    readWithPrompt(fieldDes) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.instance.readPrompt(fieldDes);
        });
    }
    /**
     * write a message
